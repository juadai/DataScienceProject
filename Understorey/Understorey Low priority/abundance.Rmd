...
Y0.Y3.guass.model = glm(Y0.Y3.Difference ~ (Treatment+ Life.Form + Gap + Fenced)^2, family = gaussian, data = rel_abundance_wide)
...
# only treatment, gap and fenced are significant, all other factors are not significant in predicting the yr_relative_abundance_difference, AIC decreases from 15883.9 to 15865
...
Y0.Y3.guass.model = glm(log(Y0.Y3.Abs_Difference) ~ (Treatment+Life.Form + Gap + Fenced)^2, family = gaussian, data = rel_abundance_wide)
summary(Y0.Y3.guass.model)
par(mfrow = c(2,2))
...
Y0.Y3.gamma.model2 = glm(Y0.Y3.Abs_Difference ~ (Treatment+Life.Form + Gap + Fenced)^2, family = Gamma, data = rel_abundance_wide)
...
# AIC decrease from 16644.58 to 16645
...
Y0.Y3.poisson.model2 = glm(Y0.Y3.Abs_Difference~ (Treatment  + Gap + Life.Form + Fenced)^2, family = poisson, data = rel_abundance_wide)
...
#AIC decrease from 16091 to 16070
...
# the model is significant at 5%, suggest that the coefficient for treatment, lifeform,gap and fenced, interaction between treatment and gap, life form and gap are not zero, Pr(>Chi) = < 2.2e-16
```
...
abline(0,1,col="red")
#fit the model
Y0.Y3.nb.model2 = glm.nb(Y0.Y3.Abs_Difference~ (Treatment  + Gap + Life.Form + Fenced)^2, data = rel_abundance_wide)
...
# AIC decrease from 16093 to 16072
...
**select model based on the AIC, we will choose the guassian model, since it gives the lowest AIC of 15865**



## diffrent modeling via prediction
```{r model_prediction_gaussian, echo=TRUE}
Y0.Y3.guass.model2 = glm(X3 ~ (X0 + Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = rel_abundance_wide)
...
Y0.Y3.gamma.model3 = glm(X3  ~ (X0 + Treatment  + Gap + Life.Form + Fenced)^2, family = Gamma(link = "log"), data = rel_abundance_wide)
summary(Y0.Y3.gamma.model3)
par(mfrow = c(2,2))

plot(Y0.Y3.gamma.model3)

Y0.Y3.poisson.model3 = glm(X3 ~ (X0 + Treatment  + Gap + Life.Form + Fenced)^2, family = poisson, data = rel_abundance_wide)
...
#AIC = 14760
# according to the AIC, we will choose the gamma model, since it has the lowest AIC of 10380,
...
Y0.Y3.guass.model2 = glm(log(X3+1) ~ (log((X0+1)/offset(Total_X0+sum)) + offset(log(Total_X3+sum))+ Treatment + Life.Form + Gap  + Fenced)^2, family = gaussian, data = abundance_df1)
summary(Y0.Y3.guass.model2)
par(mfrow = c(2,2))
plot(Y0.Y3.guass.model2)

rel_abundance_wide$X3 = log(rel_abundance_wide$X3+ 1)
Y0.Y3.guass.model3 = glm(log(X3+1) ~ (log(X0+1)-offset(log(Total_X0+sum)) + offset(log(Total_X3+sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = abundance_df1)
...
Y0.Y3.gamma.model3 = glm(log(X3+2) ~ (log(X0+2)-offset(log(Total_X0+2*sum)) + offset(log(Total_X3+2*sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = Gamma(link = "log"), data = abundance_df1)
summary(Y0.Y3.gamma.model3)
par(mfrow = c(2,2))

plot(Y0.Y3.gamma.model3)

Y0.Y3.poisson.model3 = glm(log(X3+1)~ (log(X0+1)-offset(log(Total_X0+sum)) + offset(log(Total_X3+sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = poisson, data = abundance_df1)
summary(Y0.Y3.poisson.model3)
par(mfrow = c(2,2))

plot(Y0.Y3.poisson.model3)

Y0.Y3.poisson.model4 = glm(X3~ (log(X0+1)-offset(log(Total_X0+sum)) + offset(log(Total_X3))+ Treatment  + Gap + Life.Form + Fenced)^2, family = poisson, data = abundance_df1)
...
Y0.Y3.select_guass.model2 <- step(Y0.Y3.guass.model2, scope=~.)
summary(Y0.Y3.select_guass.model2)
# AIC: 4285.79 to 4280.1

Y0.Y3.select_gamma.model3 <- step(Y0.Y3.gamma.model3, scope=~.)
summary(Y0.Y3.select_gamma.model3)
# AIC:6203.7 to 6179.2

Y0.Y3.select_poisson.model3 <- step(Y0.Y3.poisson.model3, scope=~.)
summary(Y0.Y3.select_poisson.model3)
# AIC: inf to inf

# according to the AIC, we will choose the gaussian model, since it has the lowest AIC of 4280,
```

```{r model_selection_prediction_gaussian_log_glmer, echo=TRUE}
rel_abundance_wide$X3 = log(rel_abundance_wide$X3+ 1)
Y0.Y3.glmer.guass1= glmer(log(X3+1) ~ log((X0 + 1)/offset(Total_X0 + sum)) + Treatment + Life.Form + Gap + Fenced + log((X0 + 1)/offset(Total_X0 + sum)):Treatment + log((X0 + 1)/offset(Total_X0 + sum)):Life.Form + log((X0 + 1)/offset(Total_X0 + sum)):Fenced + Treatment:Life.Form + Treatment:Gap + Life.Form:Fenced + offset(log(Total_X3 + sum)) + (1|Plot.Number), family = gaussian, data = abundance_df1)
...
#(1|Plot.Number) is significant, p-value = 2.467e-10

Y0.Y3.glmer.guass2= glmer(log(X3+1) ~  log((X0 + 1)/offset(Total_X0 + sum)) + Treatment + Life.Form + Gap + Fenced + log((X0 + 1)/offset(Total_X0 + sum)):Treatment + log((X0 + 1)/offset(Total_X0 + sum)):Life.Form + log((X0 + 1)/offset(Total_X0 + sum)):Fenced + Treatment:Life.Form + Treatment:Gap + Life.Form:Fenced + offset(log(Total_X3 + sum)) + (1|Plot.Number) +(log(X0 + 1)|Plot.Number) , family = gaussian, data = abundance_df1)
summary(Y0.Y3.glmer.guass2)
par(mfrow = c(2,2))
plot(Y0.Y3.glmer.guass2)
anova(Y0.Y3.glmer.guass1, Y0.Y3.glmer.guass2)
#(log(X0 + 1)|Plot.Number) not significant, p-value= 0.1596

Y0.Y3.glmer.guass3= glmer(log(X3+1) ~ log((X0 + 1)/offset(Total_X0 + sum)) + Treatment + Life.Form + Gap + Fenced + log((X0 + 1)/offset(Total_X0 + sum)):Treatment + log((X0 + 1)/offset(Total_X0 + sum)):Life.Form + log((X0 + 1)/offset(Total_X0 + sum)):Fenced + Treatment:Life.Form + Treatment:Gap + Life.Form:Fenced + offset(log(Total_X3 + sum)) + (1|Plot.Number) + (1|Quadrat.Number), family = gaussian, data = abundance_df1)
...
#(1|Quadrat.Number) is not significaant, p-value = 0.1249
...
#gap, fenced, X0, lifeform are all significant,both treatment are significant in this case
```
...
model1 <- glm(log(Total_X6) ~log(Total_X3) + log(Total_X0) + Treatment  + Gap + Life.Form + Fenced, family = gaussian, data = abundance_df1)
summary(model1)
par(mfrow = c(2,2))
plot(model1)

model2 <- glmer(log(Total_X6) ~log(Total_X3) + log(Total_X0) + Treatment  + Gap + Life.Form + Fenced + (1|Plot.Number) + (1| Quadrat.Number), family = gaussian, data = abundance_df1)
summary(model2)
par(mfrow = c(2,2))
plot(model2)
...
Y3.Y6.glmer.guass3= glmer(log(X3+1) ~ -offset(log(Total_X3+sum)) + offset(log(Total_X6+sum))+ Treatment  + Gap + Life.Form + Fenced + (1|Plot.Number) + (log(X0 + 1)|Plot.Number), family = gaussian, data = abundance_df1)
...
#gap, X0,fenced and lifeform are all significant, using the random effect (1|Plot.Number) + (log(X0 + 1)|Plot.Number)
```
...
Y0.Y6.guass.model = glm(Y0.Y6.Difference ~ (Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = rel_abundance_wide)
...
Y0.Y6.gamma.model2 = glm(Y0.Y6.Abs_Difference ~ (Treatment  + Gap + Life.Form + Fenced)^2, family = Gamma, data = rel_abundance_wide)
...
# AIC decrease from 16557.2 to 16486
...
Y0.Y6.poisson.model2 = glm(Y0.Y6.Abs_Difference~ (Treatment  + Gap + Life.Form + Fenced)^2, family = poisson, data = rel_abundance_wide)
...
#AIC decrease from 16037.65 to 15976
...
Y0.Y6.guass.model2 = glm(log(X6+1) ~ (log((X0+1)/offset(Total_X0+sum)) + offset(log(Total_X6+sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = abundance_df1)
summary(Y0.Y6.guass.model2)
par(mfrow = c(2,2))
plot(Y0.Y6.guass.model2)


Y0.Y6.gamma.model3 = glm(log(X6+2) ~ (log(X0+2)-offset(log(Total_X0+2*sum)) + offset(log(Total_X6+2*sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = Gamma(link = "log"), data = abundance_df1)
...
# according to the AIC, we will choose the gaussian model, since it has the lowest AIC of 4536,
#gap, fenced, X0 and lifeform are all significant,both treatment are not significant in this case, also both of them has a positive effect on the relative abundance for yr6
```


```{r model_prediction_gaussian_log_Y036, echo=TRUE}
Y0.Y6.guass.model3 = glm(log(X6+1) ~ (log((X0+1)/offset(Total_X0+sum)) + log((X3+1)/offset(Total_X3+sum)) +  offset(log(Total_X6+sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = abundance_df1)
summary(Y0.Y6.guass.model3)
par(mfrow = c(2,2))
plot(Y0.Y6.guass.model3)


Y0.Y6.gamma.model4 = glm(log(X6+3) ~ (log(X0+3)-offset(log(Total_X0+3*sum)) + log(X3+3)-offset(log(Total_X3+3*sum)) +  offset(log(Total_X6+3*sum))+ Treatment  + Gap + Life.Form + Fenced)^2, family = Gamma(link = "log"), data = abundance_df1)
...
# according to the AIC, we will choose the latter gaussian model, that is the one also included the X3 data, since it has the lowest AIC of 4292,
...
```{r model_selection_prediction_gaussian_log_glmer, echo=TRUE}
Y0.Y6.glmer.guass1= glmer(log(X6+1)  ~ log((X0 + 1)/offset(Total_X0 + sum)) + log((X3 + 1)/offset(Total_X3 + sum)) + Treatment + Gap + Life.Form + Fenced + log((X3 + 1)/offset(Total_X3 + sum)):Treatment + log((X3 + 1)/offset(Total_X3 + sum)):Life.Form + log((X3 + 1)/offset(Total_X3 + sum)):Fenced + Gap:Fenced + Life.Form:Fenced + offset(log(Total_X6 + sum)) + (1|Plot.Number), family = gaussian, data = abundance_df1)
...
#(1|plot.number) significant, p-value =< 2.2e-16


Y0.Y6.glmer.guass2= glmer(log(X6 + 1) ~ log((X0 + 1)/offset(Total_X0 + sum)) + log((X3 + 1)/offset(Total_X3 + sum)) + Treatment + Gap + Life.Form + Fenced + log((X3 + 1)/offset(Total_X3 + sum)):Treatment + log((X3 + 1)/offset(Total_X3 + sum)):Life.Form + log((X3 + 1)/offset(Total_X3 + sum)):Fenced + Gap:Fenced + Life.Form:Fenced + offset(log(Total_X6 + sum))+ (1|Plot.Number) + (log(X0 + 1)|Plot.Number) +(log(X3 + 1)|Plot.Number) , family = gaussian, data = abundance_df1)
summary(Y0.Y6.glmer.guass2)
par(mfrow = c(2,2))
plot(Y0.Y6.glmer.guass2)
anova(Y0.Y6.glmer.guass1, Y0.Y6.glmer.guass2)
#(log(X0 + 1)|Plot.Number) +(log(X3 + 1)|Plot.Number) both significant, p-value =  2.864e-05
 

Y0.Y6.glmer.guass3= glmer(log(X6 + 1) ~ log((X0 + 1)/offset(Total_X0 + sum)) + log((X3 + 1)/offset(Total_X3 + sum)) + Treatment + Gap + Life.Form + Fenced + log((X3 + 1)/offset(Total_X3 + sum)):Treatment + log((X3 + 1)/offset(Total_X3 + sum)):Life.Form + log((X3 + 1)/offset(Total_X3 + sum)):Fenced + Gap:Fenced + Life.Form:Fenced + offset(log(Total_X6 + sum)) + (1|Plot.Number) +(log(X3 + 1)|Plot.Number) + (log(X0 + 1)|Plot.Number)+ (1|Quadrat.Number), family = gaussian, data = abundance_df1)
summary(Y0.Y6.glmer.guass3)
print(Y0.Y6.glmer.guass3, correlation=TRUE)
par(mfrow = c(2,2))
plot(Y0.Y6.glmer.guass3)
anova(Y0.Y6.glmer.guass3, Y0.Y6.glmer.guass2)
#(1|Quadrat.Number) significant, p-value = 0.03572


# according to the Chi-sq value, we will select the random effect (1|plot), (1|quadrat), (log(X0+1)|plot), (log(X3+1)|plot), (1|Quadrat.Number)
```
...
## summary using total aundance
```{r summary,echo=TRUE}
model1 <- glm(log(Total_X3)  ~ (log(Total_X0) + Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = abundance_df1)
summary(model1)
par(mfrow = c(2,2))
plot(model1)
select_model1 <- step(model1, scope = ~.)
summary(select_model1)

model2 <- glmer(log(Total_X3) ~ log(Total_X0) + Treatment + Gap + Fenced + log(Total_X0):Treatment + log(Total_X0):Gap + log(Total_X0):Fenced + Treatment:Gap + Treatment:Fenced + (1|Plot.Number) , family = gaussian, data = abundance_df1)
summary(model2)
par(mfrow = c(2,2))
plot(model2)
anova( model2, select_model1)
#(1|plot.number) is significant, p-value = < 2.2e-16

model3 <- glmer(log(Total_X3) ~ log(Total_X0) + Treatment + Gap + Fenced + log(Total_X0):Treatment + log(Total_X0):Gap + log(Total_X0):Fenced + Treatment:Gap + Treatment:Fenced + (1+log(Total_X0)+Treatment +Gap+ Fenced|Plot.Number)+ (1|Quadrat.Number), family = gaussian, data = abundance_df1)
summary(model3)
par(mfrow = c(2,2))
plot(model3)
anova( model3, model2)
#(1+log(Total_X0)+Treatment +Gap+ Fenced|Plot.Number) + (1|Quadrat.Number), p-value< 2.2e-16

model4 <- glmer(log(Total_X3) ~ log(Total_X0) + Treatment + Gap + Fenced + log(Total_X0):Treatment + log(Total_X0):Gap + log(Total_X0):Fenced + Treatment:Gap + Treatment:Fenced + (1+log(Total_X0)+Treatment+Gap + Fenced|Plot.Number) + (1+log(Total_X0)+Treatment+Gap + Fenced |Quadrat.Number), family = gaussian, data = abundance_df1)
summary(model4)
par(mfrow = c(2,2))
plot(model4)
anova( model3, model4)

#(1+log(Total_X0)+Treatment+Gap + Fenced |Quadrat.Number), p-value< 2.2e-16



model5 <- glm(log(Total_X6) ~(log(Total_X3) + log(Total_X0) + Treatment  + Gap + Life.Form + Fenced)^2, family = gaussian, data = abundance_df1)
summary(model5)
par(mfrow = c(2,2))
plot(model5)
select_model5 <- step(model5, scope = ~.)
summary(select_model5)


model6 <- glmer(log(Total_X6) ~ log(Total_X3) + log(Total_X0) + Treatment + Gap + Fenced + log(Total_X3):log(Total_X0) + log(Total_X3):Treatment + log(Total_X3):Fenced + log(Total_X0):Gap + log(Total_X0):Fenced + Treatment:Gap + Gap:Fenced+ (1|Plot.Number) + (1| Quadrat.Number), family = gaussian, data = abundance_df1)
summary(model6)
par(mfrow = c(2,2))
plot(model6)
anova(model6, select_model5)
#(1|Plot.Number) + (1| Quadrat.Number) both significant, p-value < 2.2e-16

model7 <- glmer(log(Total_X6) ~ log(Total_X3) + log(Total_X0) + Treatment + Gap + Fenced + log(Total_X3):log(Total_X0) + log(Total_X3):Treatment + log(Total_X3):Fenced + log(Total_X0):Gap + log(Total_X0):Fenced + Treatment:Gap + Gap:Fenced+ (1+log(Total_X3) + log(Total_X0) + Treatment + Gap + Fenced|Plot.Number) + (1+log(Total_X3) + log(Total_X0) + Treatment + Gap + Fenced| Quadrat.Number), family = gaussian, data = abundance_df1)
summary(model7)
par(mfrow = c(2,2))
plot(model7)
anova(model7, model6)
#(1+log(Total_X3) + log(Total_X0) + Treatment + Gap + Fenced|Plot.Number) + (1+log(Total_X3) + log(Total_X0) + Treatment + Gap + Fenced| Quadrat.Number) are significant
```
