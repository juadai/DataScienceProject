...
```
```{r}
# hypothesis test for InGap = not InGap
C <- c(0,0,0,0,1,1)
linearHypothesis(mixed2.0, C) # Pr(>Chisq) = 1.579e-05, reject H0, so InGap != not InGap
```
...
Results of the hypothesis tests says that InGap is significantly different from not-InGap.

Interpretation:
InGap trees have larger tree gorwth than not InGap trees.
