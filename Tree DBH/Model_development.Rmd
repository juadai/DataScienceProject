...
predictors: log(DBH.year.0), Treatment, Large, log(DBH.year.0):Large, Treatment:Large
```{r}
model2 = glm(log(DBH.year.6)~log(DBH.year.0)+Treatment+Large+log(DBH.year.0):Large+Treatment:Large, data = DBHdatarm)
summary(model2)

tss = sum((log(DBHdatarm$DBH.year.6)-mean(log(DBHdatarm$DBH.year.6)))^2) #TSS
rss = sum((log(DBHdatarm$DBH.year.6)-model2$fitted.values)^2) #RSS
(tss-rss)/tss #R^2 = 0.9783684

#par(mfrow=c(2,2))
plot(model2)
#par(mfrow=c(1,1))
plot(log(DBHdatarm$DBH.year.6), model2$fitted.values, type = "p", col = DBHdatarm$Treatment)
...
```{r}
model3 = glm(log(DBH.year.6)~log(DBH.year.0)+Treatment+InGap+log(DBH.year.0):InGap, data = DBHdatarm)
summary(model3)
...
legend("bottomright", legend = c("Control", "T1", "T2"), fill = c("black", "red", "green"))
abline(0,1)
plot(log(DBHdatarm$DBH.year.6), model3$fitted.values, type = "p", col = DBHdatarm$InGap)
...
glm with gamma link for DBHpositive$Tree.growth
```{r}
...
model1.2 = glm(Tree.growth~DBH.year.0+Treatment+Large+DBH.year.0:Treatment+DBH.year.0:Large, family = "Gamma", data = DBHpositiverm)
summary(model1.2)

tss = sum((DBHpositiverm$Tree.growth-mean(DBHpositiverm$Tree.growth))^2) # TSS
rss = sum((DBHpositiverm$Tree.growth-model1.2$fitted.values)^2) # RSS
(tss-rss)/tss # R^2 = 0.4002613

plot(model1.2)
plot(DBHpositiverm$Tree.growth, model1.2$fitted.values, type = "p", col = DBHpositiverm$Treatment)
abline(0, 1)
```
...
model1.3 = lmer(Tree.growth~DBH.year.0+Treatment+Large+DBH.year.0:Large+Treatment:Large+(DBH.year.0+Treatment+Large+DBH.year.0:Large+Treatment:Large|Plot), data = DBHpositiverm)
summary(model1.3)
y.hat <- fitted(model1.3) # Fitted values
int.hat <- ranef(model1.3)[[1]][[1]] # Predicted intercepts
res.hat <- residuals(model1.3) # Estimated residuals
tss = sum((DBHpositiverm$Tree.growth-mean(DBHpositiverm$Tree.growth))^2) # TSS
