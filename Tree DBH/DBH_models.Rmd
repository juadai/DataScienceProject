...
# fit glm for DBH.year.6~DBH.year.0 + categorical predictors
```{r}
...
- Both glm3.0 and glm4.0 fit the data very well with very high R^2 values.
- Although residuals are not very small, and data still has heavy tails in the Normal Q-Q plots, the models still fit very well. This is due to the very strong linear correlation between the response variable DBH.year.6 and one predictor DBH.year.0.
- However, even though with the prescence of DBH.year.0, the other two categorical predictors (Treatment, Large) still are very significant.
- The result formula is the same as formula used in models for Tree.growth. The estimated parameters satisfy all the hypotheses (in a similar way as previous models): 
...
# H_0: No significant difference in the fit of models 2 and 3
anova(log_model_02, log_model_03, test='Chi')
# Reject H_0 ... the larger model (2) is useful
```
```{r}
plot(log_model_02)
